...
My Bookings
          ...
          <li class="nav-item has-treeview menu-open">
            <a href="viewbill.php" class="nav-link active">
...
My Bill
  ...
  <div class="content-wrapper">
   ...
   <section class="content">
	 ...
	 error_reporting(0);
	  $a=$_SESSION['username'];
	$query=("SELECT * from reqest where uid='$a' AND status='booked'");
...
{
	$sql=("SELECT * from property where pname='{$data['pname']}' ");

$res = mysqli_query($con,$sql); // selecting data through mysql_query()
$data1 = mysqli_fetch_array($res);
$price=$data1['price'];
$startTimestamp = strtotime($data['ft']);
    $endTimestamp = strtotime($data['rt']);
    $timeDifference = $endTimestamp - $startTimestamp;

    // Convert the time difference to minutes or hours as needed
    $timeInHours = round($timeDifference / 3600, 2);
    $total = $timeInHours * $price;
...
?>
                ...
                <tr>
                <form action="payb.php"  method="post">
                <input type="hidden" name="p5" readonly class="form-control" value="<?php echo $data['rid']; ?>">

                 <input type="hidden" name="p1" readonly class="form-control" value="<?php echo $data['pname']; ?>">
                    <td><?php echo $data['pname']; ?> </td>
				
                   <td><?php echo $data['ft']; ?> </td>

                   <td><?php echo $data['rt']; ?> </td>
                   <input type="hidden" name="p0" readonly class="form-control" value="<?php echo $data['uid']; ?>">
                   <input type="hidden" name="p2" readonly class="form-control" value="<?php echo $total; ?>">
				    <td>&#8377 <?php echo $total; ?> </td>
                    <td> Phone Pe: 1234567890<input type="text" required name="p3"  class="form-control"></td>

                    <td><button type="submit" class="btn btn-block bg-gradient-success ">Paid</button></td>
